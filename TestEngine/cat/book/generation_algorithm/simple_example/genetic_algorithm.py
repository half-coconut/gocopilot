...
from demo import chromosome

C = TypeVar('C', bound=chromosome.Chromosome)


class GeneticAlgorithm(Generic[C]):
    SelectionType = Enum("SelectionType", "ROULETTE TOURNAMENT")

    def __init__(self, initial_population: List[C],
                 threshold: float,
    ...
    # and return the best in dividual found
    def run(self) -> C:
        ...
        for generation in range(self._max_generations):
            ...
            for i in self._population:
                print(f"变异后，self._population 的 individual: {i}")

...
func InitWebServer() *App {
	...
	apiCache := cache.NewAPICache(cmdable)
	apiRepository := repository.NewAPIRepository(apidao, apiCache, userCache, loggerV1, userRepository)
	apiService := service.NewAPIService(apiRepository, loggerV1)
	taskDAO := dao.NewGORMTaskDAO(db, loggerV1)
	taskCache := cache.NewTaskCache(cmdable)
	taskRepository := repository.NewCacheTaskRepository(taskDAO, taskCache, loggerV1, userRepository, apiRepository, userCache)
	...
	cronJobRepository := repository.NewCacheCronJobRepository(cronJobDAO, userCache, loggerV1, userRepository)
